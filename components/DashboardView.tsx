    ...
    <div className="animate-fade-in pb-12">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
        <div>
          <button 
            onClick={onBack}
            className="flex items-center text-sm text-gray-500 hover:text-apple-dark mb-2 transition-colors"
          >
        ...
        <div className="flex gap-3">
          <button className="flex items-center gap-2 px-5 py-2.5 bg-white border border-gray-200 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors shadow-sm">
            <Download className="w-4 h-4" />
            Export PDF
          </button>
          <button className="flex items-center gap-2 px-5 py-2.5 bg-apple-dark text-white rounded-lg text-sm font-medium hover:bg-black transition-colors shadow-lg shadow-gray-400/20">
            <Download className="w-4 h-4" />
            Export Excel
          </button>
        </div>
      ...
      {/* AI Insight Card */}
      <GlassCard className="mb-8 p-6 bg-gradient-to-br from-white/80 to-blue-50/50 border-blue-100">
            ...
            {loadingAnalysis ? (
              ...
              <div className="space-y-3">
                ...
                <div className="flex flex-wrap items-center gap-4 mt-2">
                  ...
                  <span className="text-sm font-medium text-apple-dark flex items-center gap-2">
                    Recommendation: {analysis.recommendation}
                  </span>
                </div>
              </div>
            ) : (
               <p className="text-gray-500">Analysis unavailable.</p>
            )}
          </div>
        </div>
      </GlassCard>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
        ...
        <GlassCard className="lg:col-span-2 p-6">
          <div className="flex items-center justify-between mb-6">
            ...
            <span className={`text-sm px-2 py-1 rounded ${isOverBudget ? 'bg-red-50 text-red-600' : 'bg-green-50 text-green-600'}`}>
    ...
    <div className="space-y-4">
      {items.map((item, i) => (
        <div key={i} className="flex items-center justify-between p-3 hover:bg-gray-50 rounded-lg transition-colors border border-transparent hover:border-gray-100">
          ...
          <span className="font-semibold text-sm text-gray-700">{item.value}</span>
        </div>
      ))}
    </div>
    <button className="w-full mt-4 py-2 text-sm text-apple-blue font-medium hover:bg-blue-50 rounded-lg transition-colors">
